...
async function updateDevice(data) {
  try {
    ...
    const req = {
      params: { deviceHost: data.deviceid, power: data.power }, // Harriets lights
    };
...
async function setupSchedule(data) {
  try {
    ...
    this.logger.debug(
      `${this._traceStack()} - Add tp-link schedule to base class schedules`,
    );
    this._addSchedule(date, data.name, updateDevice, data);
...
async function setupSchedules() {
  try {
    this.logger.debug(`${this._traceStack()} - Setup daily re-set schedules`);

    // Cancel any existing schedules
    this.logger.debug(`${this._traceStack()} - Cancel all existing schedules`);
    ...
    // Setup schedules
    results.rows.map((info) => setupSchedule.call(this, info));
    return true;
  } catch (err) {
    this.logger.error(`${this._traceStack()} - ${err.message}`);
    return false;
