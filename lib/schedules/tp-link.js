  ...
  if (data.device[0].room === 'kids' && data.power) {
    ...
    if (data.override) {
      ...
      try {
        ...
        sunsetData = await this._callAlfredServiceGet(url);
      } catch (err) {
        this.logger.error(`${this._traceStack()} - ${err.message}`);
      }
      ...
      if (sunsetData instanceof Error)
        this.logger.error(
          `${this._traceStack()} - ${sunsetData.response.data.error}`,
        );

      if (
        sunsetData instanceof Error ||
        typeof sunsetData === 'undefined' ||
        sunsetData === null
      ) {
        date.setHours(data.hour);
        date.setMinutes(data.minute);
        ...
        if (dateFormat(sunSet, 'HH:MM') < '17:00') {
          this.logger.debug(
            `${this._traceStack()} - Resetting time due to sunset being before 5pm`,
          );
          sunSet.setHours(17);
          sunSet.setMinutes(0);
        }
